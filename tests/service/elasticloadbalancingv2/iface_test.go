...
func TestIClient(t *testing.T) {
    ...
    t.Run("TestModifyIpPools", func(t *testing.T) {
        input := &elasticloadbalancingv2.ModifyIpPoolsInput{}
        output := &elasticloadbalancingv2.ModifyIpPoolsOutput{}

        mockClient.On("ModifyIpPools", ctx, input).Return(output, nil)

        result, err := mockClient.ModifyIpPools(ctx, input)
        assert.NoError(t, err)
        assert.Equal(t, output, result)
