...
// tests for the autoscaling service interface for 
// this ../../../service/autoscaling/iface.go
...
func TestIClient(t *testing.T) {
    ...
    t.Run("TestLaunchInstances", func(t *testing.T) {
        input := &autoscaling.LaunchInstancesInput{}
        output := &autoscaling.LaunchInstancesOutput{}

        mockClient.On("LaunchInstances", ctx, input).Return(output, nil)

        result, err := mockClient.LaunchInstances(ctx, input)
        assert.NoError(t, err)
        assert.Equal(t, output, result)
